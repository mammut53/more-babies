...
import io.github.mammut53.more_babies.world.entity.monster.guardian.ElderGuardianGroupData;
...
public abstract class ElderGuardianMixin extends Guardian {
    ...
    private static final EntityDimensions more_babies$BABY_DIMENSIONS = EntityDimensions
            .scalable(0.99875F, 0.99875F)
            .withEyeHeight(0.499375F);

    protected ElderGuardianMixin(final EntityType<? extends Guardian> type, final Level level) {
        super(type, level);
    }
    ...
    public SpawnGroupData finalizeSpawn(final @NonNull ServerLevelAccessor level, final @NonNull DifficultyInstance difficulty, final @NonNull EntitySpawnReason spawnReason, @Nullable final SpawnGroupData spawnGroupData) {
        ...
        if (groupData == null) {
            groupData = new ElderGuardianGroupData(more_babies$getSpawnAsBabyOdds(random));
        }

        if (groupData instanceof ElderGuardianGroupData(boolean isBaby) && isBaby) {
            this.setBaby(true);
        }
    ...
    private static boolean more_babies$getSpawnAsBabyOdds(final RandomSource random) {
        return random.nextFloat() < MoreBabiesConfig.elderGuardianBabySpawnChance;
    }
